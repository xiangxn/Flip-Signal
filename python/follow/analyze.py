#!/usr/bin/env python3
"""
Follow 线特征筛选 —— 决策时刻预判 only/both 类别（独立模块，只服务 follow）。

观测单位: 事件内时间顺序首个 0.7 穿越（确认 T+2 ticks 的成交特征）。
标签:    only = 整窗内仅穿越侧出现过 >0.7（follow 的领域, 该侧 ~97%+ 赢）
         both = 对侧后来也穿越过 0.7（follow 必亏, fade 的领域）
         ⚠️ 标签是整窗未来信息，仅作训练目标，绝不进特征。
特征:    全部 ≤ 决策时刻已知（穿越 tick 或确认 tick）。
输出:    每特征分桶 × only 率 × follow EV × χ²；top 桶按天稳定性。
...
def extract_obs(events: list[dict], outcome_field: str) -> list[dict]:
    ...
    for event in events:
        ...
        o = {
            ...
            "side": side,
            "cls": "only" if len(crossed) == 1 else "both",
            "rem": s["remaining_sec"],
            "elapsed": 300 - s["remaining_sec"],
            "trigger_bid": s[this_key],
            "fade_fill": (1.0 - conf[this_key]) if conf else None,
...
FEATURES = [
    ...
    ("follow_fill", "follow 成交价", [("<0.75", -1e9, 0.75), ("0.75-0.80", 0.75, 0.80),
                                      ("0.80-0.85", 0.80, 0.85), (">0.85", 0.85, None)]),
    ("fade_fill", "fade 成交价(对照)", [("<0.20", -1e9, 0.20), ("0.20-0.25", 0.20, 0.25),
    ...
    ("vol_10s", "波动率10s", None),   # None = 动态三分位
...
def screen(obs: list[dict], key: str, buckets, base_only: float, base_fev: float) -> None:
    vals = [o[key] for o in obs if o.get(key) is not None]
    if len(vals) < 30:
        print(f"\n  【{key}】 有效样本不足 ({len(vals)})")
        return
    if buckets is None:  # 动态三分位
        srt = sorted(vals)
        q1, q2 = srt[len(srt) // 3], srt[2 * len(srt) // 3]
        buckets = [("低T1", -1e9, q1), ("中T2", q1, q2), ("高T3", q2, None)]

    print(f"\n  【{key}】 (基线 only 率 {base_only * 100:.1f}%, follow EV {base_fev:+.4f})")
    print(f"  {'分桶':<16s} {'n':>5s} {'only率':>8s} {'Δ':>7s} {'followEV':>10s}")
    table = []
    for label, lo, hi in buckets:
        b = [o for o in obs if (v := o.get(key)) is not None and lo <= v and (hi is None or v < hi)]
        if not b:
            continue
        only = sum(1 for o in b if o["cls"] == "only") / len(b)
        fev = sum(((1.0 - o["follow_fill"]) if o["won"] else -o["follow_fill"])
                  for o in b if o["follow_fill"] is not None) / max(1, len([o for o in b if o["follow_fill"] is not None]))
        n_f = len([o for o in b if o["follow_fill"] is not None])
        print(f"  {label:<16s} {len(b):>5d} {only * 100:>7.1f}% {(only - base_only) * 100:>+6.1f}pp "
              f"{fev:>+9.4f} (n={n_f})")
        table.append([sum(1 for o in b if o["cls"] == "only"),
                      sum(1 for o in b if o["cls"] == "both")])
...
def main():
    parser = argparse.ArgumentParser(description="Follow 线特征筛选（独立）")
    ...
    if not obs:
        print("无观测。")
        sys.exit(1)
    only_n = sum(1 for o in obs if o["cls"] == "only")
    print(f"数据: {args.data}  |  事件 {len(events)}  |  首个穿越观测 {len(obs)}  |  "
          f"only {only_n} ({only_n / len(obs) * 100:.1f}%) / both {len(obs) - only_n}")
    base_only = only_n / len(obs)
    base_fev = sum(((1.0 - o["follow_fill"]) if o["won"] else -o["follow_fill"])
                   for o in obs if o["follow_fill"] is not None)
    n_f = len([o for o in obs if o["follow_fill"] is not None])
    base_fev /= max(1, n_f)

    print("=" * 62)
    print("  Follow 线: 预判 only 类（follow 赢 97%+ 的窗口）的特征筛选")
    print("=" * 62)
    for key, label, buckets in FEATURES:
        screen(obs, key, buckets, base_only, base_fev)
