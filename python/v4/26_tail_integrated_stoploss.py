...
🔒 **冻结声明（2026-09-26）**：本脚本是 **09-25 那次止损评估的存档**，定格在当时的
   23 号口径上——**T=150 段价格腿仍是 `≥ 0.80`**。23 与引擎已在 2026-09-26 改成
   T=150 严格大于（决策 #26），故本脚本的 ORACLE 自检从此对的是**冻结基线**而不是
   当天的 23；自检通过只说明「拷贝忠实于 09-25 的基线」，**它的四个数字与 Δ 都不是
   现行引擎的数字**。要重跑止损评估须先把 r5 的价格腿改成 `r5(r, strict_price=True)`
   并同步 ORACLE —— 在此之前不要把它当现行策略的对照读数。
...
# 23 号脚本的 oracle 数字（自检用; **09-25 冻结值**——2026-09-26 起 23 已改 T=150
# 严格大于, 见文件头「冻结声明」。要重跑止损评估须先同步本表与 r5 的 strict_price）
ORACLE = {
...
def main():
    ...
    print("\n" + "=" * 100)
    print("五、parity 断言表（= **09-25 冻结的** 23 号 oracle, 逐位自检）")
    print("=" * 100)
    ...
    print(f"  // 基线自检: {'✅ 与 09-25 冻结基线逐位一致（拷贝未引入偏差）' if ok_all else '❌ 与冻结基线不一致——拷贝过程出错, 上面的止损 Δ 不可信'}")
    print("  // ⚠️ 本表是 09-25 存档: T=150 价格腿仍是 ≥ 0.80（现行引擎为严格大于, 决策 #26）")
