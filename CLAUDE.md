...
24. **持仓监察（只记录）+ 止损评估：离线支持、能不能成交是唯一的坎；成交量维度已否**（2026-09-25，
    ...
    - **止损腿规格（未落引擎）**：触发 = 持仓侧 `bid < 0.30 ∧ dev < −20 美元`；出场 = 触发
      ...
      （bid 0.25~0.30 × dev −20 整片同号），不是单点拟合。14 天 2U/注：基线 **+35.67U**
    ...
    - **与既有决策的关系**：**不影响 parity 红线**（纯增量：新前缀、新纯函数、`cmd/tail`
      ...
      ⚠️ `internal/dashboard/tail/index.html` 有一处**本次无关的**未提交编辑（标题
      「扫尾盘 监控」→「Tail 监控」）会让 `TestTailRoutes` 失败——它断言页面含「扫尾盘」。
