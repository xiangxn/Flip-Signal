...
// 修正行合并进事件行（官方结算口径覆盖推送/流值口径），原子重写数据文件。
//
// 只对**推送缺失**的窗口有活干（默认口径是精确边界推送，与官方逐位同源）。
// 本工具与资产无关——只认目录，BTC 用 data/btc、ETH 用 data/eth。
//
...
//	go run ./cmd/compact -output data/eth -all       # ETH 全部
package main
