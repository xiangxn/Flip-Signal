...
> ⚠️ **取值口径已被 `docs/dog020_anchor_upgrade_2026-09-18.md` 取代（2026-09-18）**：
> 本文的 **§0 / §2（2.1~2.3）/ §4** 描述的「官方优先 + 窄窗口推送直采」「`abs` 到达龄判据」
> 「锚缺失才起通道」「`anchor_src=push`」**均不再成立**——现在是**每窗常驻的锚升级通道**：
> 推送缓存按**评估时刻**重选（+1s 起每秒）+ 官方 open 在 +2/+5/+10/+20/+40s 轮询，
> `anchor_src=official|stream`。**§1（锚缺失成因）、§2.4 的原则（恢复期 tick 照常喂 ring
> 只闸触发判定）、§2.5 的红线（anchor=0 不得落盘）、§3（实现映射的历史）、§5.1（btreplay
> 625 笔镜像红线）、§6（频率实测）、§9（σ 冷启动 / 假 late 两个顺带发现）仍然有效。**
>
...
> ⚠️ **本表已被 `docs/dog020_anchor_upgrade_2026-09-18.md` §0 取代**（2026-09-18）：
> 「取值来源 = 官方优先 + 窄窗口推送」「锚缺失才起恢复通道」两条不再成立。保留作历史。
...
> ⚠️ **§2.1~§2.3 已被 `docs/dog020_anchor_upgrade_2026-09-18.md` 取代**（2026-09-18）：
> 取值不再区分「锚缺失窗口」——**每窗**都按评估时刻从推送缓存重选 + 官方轮询
> （+2/+5/+10/+20/+40s），判据是**评估时刻偏移**而非到达龄。**§2.4 的原则仍在**
> （升级期 tick 照常喂 ring、只闸触发判定），**§2.5 的红线仍在**（永不落 `anchor=0`
> 的观测行），但「3 次失败」的触发条件已改（官方全失败 → 沿用流值锚；只有连 t=0
> 初值都没有才不观测）。下面文字保留作历史。
...
> ⚠️ **字段口径已被 `docs/dog020_anchor_upgrade_2026-09-18.md` §4 取代**（2026-09-18）：
> `anchor_src` 取值改 **`official|stream`**（原 `push`），并新增 **`anchor_init`**（t=0 初值）
> 与 **`anchor_pick_ms`**（评估时刻偏移）；`anchor_recovered_ms` 语义由「恢复时刻」扩为
> 「升级时刻」。下面 schema 保留作历史（字段名仍在，取值语义已变）。
