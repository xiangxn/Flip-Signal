...
### 9.2 顺带发现 ②：冷启动首窗恒 `no_hist`（**本次改动之外**，更深的老问题）

σ 预热在重启后要 10~30s（本地窗口陈旧 → 走官方网络预热，实测本次 26s，含 429 退避），
而 `engine.BeginWindow(anchor, histBps)` 在**窗口开始时**就把 σ 冻住了，热更新路径
不存在 → **边界落在预热完成之前的那个窗口，整窗 σ=0**，任何触底都被 `no_hist` 拒，
...
（`data/v4` 现存 2 行 `hist_bps=0` 观测，均为冷启动窗口。09-15 复验的主窗
09-04~09-16 恰好未覆盖 09-03 那行，09-16 这行是本次冒烟新增的——**两行都不是锚缺失，
是 σ 冷启动**。）

可选修法（均需单独评估，**不在本改动范围**）：① σ 预热期间不起窗口（等预热完再进边界）；
② 预热完成后回填引擎 σ（`SetHistBps` 式热更新）——但这会改动「窗口级常量 σ」的既有语义，
需与回测口径对照；③ 预热未完成时该窗整体标记跳过（如 `skip=preheat`）而非产出观测行。
