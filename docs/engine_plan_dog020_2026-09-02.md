...
1. **触发** = 首个「有效 tick」上某侧 ask 满足 `0 < ask ≤ 0.20`。有效 = book_latency_ms ≤ 300（MAX_LAT）且 **Up(=yes) 侧** bid/ask > 0（脚本 :79 只查 yes 报价，**不检查 no/Down 报价**；缺失 ask 经 `or 1` 语义不触发——Go 侧盘口缺失为 0，触发条件必须写 `0 < ask`，ask=0 绝不能触发）
