...
no 侧浅洞带放宽到 (−1, 0)（sgn 口径同 §3），yes 侧不变——no 触底到 0.2 时现货几乎总已
跑出 ≥0.5σ 坑（带内 14 天仅 38 例 vs yes 207 例），Down 落后者浅砸罕见 → 纸面信号全为
yes；深度放宽用于观察补量与 EV 摊薄。14 天结果（01 脚本四规则报告，明细导出
`python/v4/data/trades_r1_noR.csv`；同列名 in_pure/in_dual 指放宽版两条，勿与基准
`trades_r1.csv` 混读）：
- no 放宽 纯现货：n=546（no 339/yes 207）WR 24.4% EV +0.606U/注 +331U/14 天
  （日正 11/14；h1 +97.8U / h2 +233.2U）
- no 放宽 双层：n=461 EV +0.622U/注 +287U/14 天
- 相对 R1 新增 no(−1, −0.5] 段：301 笔 WR 20.6% EV +0.22U/注（+67U）——薄正扩量，每笔 EV 摊薄
- 附：带内 no 子集 n=38 WR 42.1% EV +2.37U/注 为全策略最佳（样本小，不当定论）
