...
// close_source / anchor_source / outcome 五个字段（官方结算口径），其余字段不变。
// ⚠️ 修正行只服务**推送缺失**的窗口（2026-09-25 同步后默认口径是精确边界推送，
// 与官方逐位同源，见 collect 包注释）：合并后 close_source/anchor_source 都变成
// official，分析侧据此把「推送命中」与「走官方兜底」的窗口分开看。
...
func CompactDay(dir, day string) (merged, orphan int, err error) {
	...
	for i := range events {
		...
		for _, k := range []string{"twap_open_price", "twap_close_price", "close_source", "anchor_source", "outcome"} {
